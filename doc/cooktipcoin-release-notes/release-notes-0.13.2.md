Cooktipcoin Core version 0.13.2 is now available from:

  <https://download.cooktipcoin.org/cooktipcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/cooktipcoin-project/cooktipcoin/issues>
...
updates will be released anymore. Without security updates, using a cooktipcoin
...
ECDSA signatures inside Cooktipcoin transactions now use validation using
...
Previous versions of Cooktipcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Cooktipcoin Core was to raise the default minimum
relay fee.

Cooktipcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Cooktipcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running cooktipcoind without having to do any manual configuration.
...
Cooktipcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Cooktipcoin Core will relay transactions with
...
entirely in Cooktipcoin Core 0.13, and it is at this time undecided whether the
...
Cooktipcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Cooktipcoin Core automatically creates a hidden service to listen on, without
manual configuration. Cooktipcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Cooktipcoin Core is listening, and
...
Cooktipcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Cooktipcoin
Core to use floating fees. By default, Cooktipcoin Core will use floating
fees.
...
(default: `0.0002` CTC/kB).

At all times, Cooktipcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) CTC.
Furthermore, Cooktipcoin Core will never create transactions paying less than
...
`cooktipcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /cooktipcoinrpc>
        ...
        # optional bypass cooktipcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< cooktipcoinrpc:<password>
    </Location>
...
Restarting cooktipcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in cooktipcoin.conf
...
cooktipcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/cooktipcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Cooktipcoin Core code base has
...
- `cooktipcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `cooktipcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Cooktipcoin scripting system that in combination with
...
Cooktipcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Cooktipcoin Core.
...
The two phases are distinct in the Cooktipcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Cooktipcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Cooktipcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Cooktipcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of cooktipcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of cooktipcoins being spent is exactly
...
- **Increased security for multisig:** Cooktipcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Cooktipcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Cooktipcoin's current ledger (called the UTXO set),
  ...
  note that Cooktipcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Cooktipcoin users to individually opt-in to almost any change in the Cooktipcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Cooktipcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Cooktipcoin
client to Cooktipcoin Core 0.13.2 to signal for segwit support. When a
...
Since Cooktipcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `cooktipcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `cooktipcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
