...
const char * const BITCOIN_CONF_FILENAME = "cooktipcoin.conf";
const char * const BITCOIN_PID_FILENAME = "cooktipcoin.pid";
...
#else
    const char* pszModule = "cooktipcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cooktipcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Cooktipcoin";
#else
    // Unix
    return pathRet / ".cooktipcoin";
#endif
#endif
}
